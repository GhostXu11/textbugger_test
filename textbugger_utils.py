...
def get_word_importances_for_whitebox(tokens, y, F, model_type, glove_vectors, embed_map, dataset):
    pred_proba = get_prediction_given_tokens(model_type, F, tokens, glove_vectors=glove_vectors, embed_map=embed_map,
                                             dataset=dataset)
    pred = y
    ...
    for ex_word, ex_tokens in excludes.items():
        ex_pred_proba = get_prediction_given_tokens(model_type, F, ex_tokens, glove_vectors=glove_vectors,
                                                    embed_map=embed_map, dataset=dataset)
