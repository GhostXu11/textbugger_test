...
import random
import pprint
from textbugger_utils import get_prediction_given_tokens, getSemanticSimilarity, transform_to_feature_vector, \
    get_word_importances_for_whitebox, generateBugs
import numpy as np
...
class WhiteBox():
    ...
    def whiteBoxAttack(self):
        ...
        W_ordered = get_word_importances_for_whitebox(self.X, self.y, self.F, self.model_type, self.glove_vectors,
                                                      self.embed_map, self.dataset)
        # print(W_ordered)
        ...
        for x_i in W_ordered:
            ...
            prediction_proba = get_prediction_given_tokens(self.model_type, self.F, x_prime,
                                                           glove_vectors=self.glove_vectors, embed_map=self.embed_map,
    ...
    def getScore(self, candidate, x_prime):
        ...
        x_prime_proba = get_prediction_given_tokens(self.model_type, self.F, x_prime, glove_vectors=self.glove_vectors,
                                                    embed_map=self.embed_map, dataset=self.dataset)
        x_prime_with_bug_proba = get_prediction_given_tokens(self.model_type, self.F, candidate,
                                                             glove_vectors=self.glove_vectors, embed_map=self.embed_map,
                                                             dataset=self.dataset)
